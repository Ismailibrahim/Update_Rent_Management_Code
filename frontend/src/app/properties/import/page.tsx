...
export default function ImportPropertiesPage() {
    ...
    reader.onload = (e) => {
      ...
      if (lines.length > headerLineIndex) {
        ...
        const headers = headerLine.split(',').map(h => {
          ...
          // Remove [REQUIRED] marker for display but keep it for reference
          return header;
        });
  ...
  const handleDownloadTemplate = async () => {
    try {
      setLoading(true);
      const response = await propertiesAPI.getImportTemplate();
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      const response = await propertiesAPI.previewImport({
        csv_data: csvData,
  ...
  const handleImport = async (skipErrors = false) => {
    try {
      ...
      if (response.data.imported > 0) {
        toast.success(`Successfully imported ${response.data.imported} properties`);
      }
      if (response.data.failed > 0) {
        toast.error(`Failed to import ${response.data.failed} properties`);
      }
  ...
  const updateFieldMapping = (csvColumn: number, propertyField: string) => {
        ...
        {step === 'upload' && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload your CSV file with property data
              </CardDescription>
            </CardHeader>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Map To Property Field</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-sm">
                        Successfully imported: <strong>{importResult.imported}</strong> properties
                      </span>
                    </div>
                    {importResult.failed > 0 && (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm">
                          Failed: <strong>{importResult.failed}</strong> properties
                        </span>
                      </div>
                    )}
