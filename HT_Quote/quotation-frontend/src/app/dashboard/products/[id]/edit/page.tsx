...
export default function EditProductPage() {
  ...
  const loadProduct = async () => {
    try {
      ...
      console.log('is_discountable value:', product.is_discountable, 'type:', typeof product.is_discountable);
      
      // Set category type based on product's category
      if (product.category && product.category.category_type) {
        setSelectedCategoryType(product.category.category_type);
      }
      ...
      const formDataToSet = {
        ...
        total_man_days: product.total_man_days !== null && product.total_man_days !== undefined ? product.total_man_days : undefined,
        pricing_model: product.pricing_model || 'one_time',
        is_man_day_based: product.is_man_day_based !== undefined ? Boolean(product.is_man_day_based) : false,
        has_amc_option: product.has_amc_option !== undefined ? Boolean(product.has_amc_option) : false,
        amc_unit_price: product.amc_unit_price || undefined,
        amc_description_id: product.amc_description_id || undefined,
        brand: product.brand || "",
        model: product.model || "",
        part_number: product.part_number || "",
        is_discountable: product.is_discountable !== undefined ? Boolean(product.is_discountable) : false,
        is_refurbished: product.is_refurbished !== undefined ? Boolean(product.is_refurbished) : false,
        is_active: product.is_active !== undefined ? Boolean(product.is_active) : true,
        sort_order: product.sort_order || 0,
      };
      
      console.log('Setting form data:', formDataToSet);
      console.log('is_discountable specifically:', {
        original: product.is_discountable,
        type: typeof product.is_discountable,
        converted: Boolean(product.is_discountable),
        final: formDataToSet.is_discountable
      });
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      setFormData({ ...formData, is_discountable: !!checked });
                    }}
                  />
                  <Label htmlFor="discountable" className="text-sm font-normal">
                    Can be Discounted (Current: {formData.is_discountable ? 'true' : 'false'})
                  </Label>
                </div>
